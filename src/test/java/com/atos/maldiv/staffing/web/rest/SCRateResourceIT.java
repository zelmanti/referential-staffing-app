...
public class SCRateResourceIT {
    ...
    public static SCRate createEntity(EntityManager em) {
        SCRate sCRate = new SCRate().code(DEFAULT_CODE).niveau(DEFAULT_NIVEAU).localisation(DEFAULT_LOCALISATION).montant(DEFAULT_MONTANT);
        return sCRate;
    }
    ...
    public static SCRate createUpdatedEntity(EntityManager em) {
        SCRate sCRate = new SCRate().code(UPDATED_CODE).niveau(UPDATED_NIVEAU).localisation(UPDATED_LOCALISATION).montant(UPDATED_MONTANT);
        return sCRate;
    }
    ...
    public void createSCRate() throws Exception {
        ...
        // Create the SCRate
        restSCRateMockMvc
            .perform(post("/api/sc-rates").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(sCRate)))
    ...
    public void getAllSCRates() throws Exception {
        ...
        restSCRateMockMvc
            .perform(get("/api/sc-rates?sort=id,desc"))
    ...
    public void getSCRate() throws Exception {
        ...
        // Get the sCRate
        restSCRateMockMvc
            .perform(get("/api/sc-rates/{id}", sCRate.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void getNonExistingSCRate() throws Exception {
        // Get the sCRate
        restSCRateMockMvc.perform(get("/api/sc-rates/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    public void updateSCRate() throws Exception {
        ...
        updatedSCRate.code(UPDATED_CODE).niveau(UPDATED_NIVEAU).localisation(UPDATED_LOCALISATION).montant(UPDATED_MONTANT);

        restSCRateMockMvc
            .perform(put("/api/sc-rates").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(updatedSCRate)))
            .andExpect(status().isOk());
    ...
    public void updateNonExistingSCRate() throws Exception {
        ...
        restSCRateMockMvc
            .perform(put("/api/sc-rates").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(sCRate)))
            .andExpect(status().isBadRequest());
    ...
    public void deleteSCRate() throws Exception {
        ...
        // Delete the sCRate
        restSCRateMockMvc
            .perform(delete("/api/sc-rates/{id}", sCRate.getId()).accept(MediaType.APPLICATION_JSON))
